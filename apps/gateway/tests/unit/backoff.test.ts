...
describe('delayForFailure', () => {
  ...
  describe('malformed retry_after_seconds falls back to the exponential policy', () => {
    // None of these are reachable through the real retryAfterMs parser
    // (backoff.test.ts's own `retryAfterMs` suite above shows it returns
    // null for anything unparseable, and never produces a negative number),
    // but delayForFailure has no way to know that: a Failure could in
    // principle be constructed by something else entirely, so it validates
    // retry_after_seconds itself rather than trusting every caller.
    const malformed: Array<[string, number]> = [
      ['NaN', Number.NaN],
      ['negative', -5],
      ['Infinity', Number.POSITIVE_INFINITY],
      ['-Infinity', Number.NEGATIVE_INFINITY],
      ['absurdly large but finite (10 days)', 10 * 24 * 60 * 60],
    ];

    for (const [label, raw] of malformed) {
      it(`falls back for ${label}, and never returns NaN or negative`, () => {
        const delay = delayForFailure(
          { kind: 'network', message: 'x', retry_after_seconds: raw },
          2,
          noJitter,
        );
        // network -> TRANSIENT_BACKOFF, attempt 2, no jitter: 1000 * 2^1.
        expect(delay).toBe(2_000);
    ...
    it('never lets a malformed value poison a rate_limited failure either', () => {
      const delay = delayForFailure(
        { kind: 'rate_limited', message: '429', retry_after_seconds: Number.NaN },
        1,
        noJitter,
      );
      expect(delay).toBe(RATE_LIMIT_BACKOFF.baseMs);
