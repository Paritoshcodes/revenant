...
describe('minimumBackoff', () => {
  ...
  it('vetoes rather than allows when the clock goes backwards', () => {
    // nowMs and lastAttemptAtMs both come from the caller (deps.now());
    // nothing here assumes time only moves forward. elapsed = nowMs -
    // lastAttemptAtMs is negative when the clock regresses, and a negative
    // elapsed is always < a positive required gap, so this already vetoes
    // by construction — asserted directly rather than left implicit.
    const decision = minimumBackoff(
      baseContext({ attemptNumber: 2, lastAttemptAtMs: NOW + 10_000 }),
      DEFAULT_GUARDRAIL_CONFIG,
    );
...
describe('circuitBreaker', () => {
  ...
  it('allows at batch start, zero settled and zero failed, without dividing by zero', () => {
    // The real shape of "batch start": settled < minSettled short-circuits
    // to allow before `failed / settled` is ever evaluated, so 0/0 never
    // happens on the path any real caller takes.
    const decision = circuitBreaker(
      baseContext({ batch: { settled: 0, failed: 0 } }),
      DEFAULT_GUARDRAIL_CONFIG,
    );
    expect(decision.verdict).toBe('allow');
    expect(decision.reason).toContain('breaker arms at');
  });
});
